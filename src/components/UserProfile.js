  ...
  // import { ref, uploadBytesResumable, getDownloadURL } from "../backend/firebase/storage";  
...
>>>>>>> origin/main
    ...
    const [previewUrls, setPreviewUrls] = useState({
      ...
      // Corresponding preview URLs
    });

    const handleSubmit = async (event) => {
  event.preventDefault();

  // Assume user's email is the unique identifier for simplicity
  ...
  const userDocRef = doc(db, "users", userEmail); // Using email as document ID for demonstration
  ...
  if (docSnap.exists()) {
    ...
    //   // Combine all the data you want to submit
    ...
    const handleFileChange = (event, fileType) => {
      ...
      if (file) {
        // Update the file object for uploading
        setSelectedFiles(prev => ({ ...prev, [fileType]: file }));
    
        // Create and update the Data URL for previewing
    ...
    const handleInputChange = (e, setData) => {
      ...
      // const handleFileChange = (event, fileType) => {
      //   setSelectedFiles({ ...selectedFiles, [fileType]: event.target.files[0] });
      // };
            ...
            <div className="bg-white p-4 shadow-md rounded-md">
              ...
              {uploadSections.map((section, index) => (
                <div key={index} className="mt-4 p-4 rounded-md border border-gray-300">
                  ...
                  <div className="mt-4 p-4 shadow-md rounded-md bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 flex flex-col">
            ...
            <div className="bg-white p-4 shadow-md rounded-md">
              ...
              {uploadEduSections.map((section, index) => (
                  <div key={index} className="border border-gray-300 mt-4 p-4 rounded-md">
                    <div className="mt-4 w-32 h-32 border border-gray-300">
                      {/* Use previewUrls for displaying the image */}
